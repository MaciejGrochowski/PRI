...
export const validationMail = mail => {
    if (mail.match("^(?![\\s\\S])") || mail.match(("^([a-zA-Z0-9_\\-\\.]+)@((\\[[0-9]{1,3}\\.[0-9]{1,3}\\.[0-9]{1,3}\\.)|(([a-zA-Z0-9\\-]+\\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\\]?)$"))
        && mail.match("^.{2,200}$")) {
        return {errorText: "", errorState: false}
    } else {
...
export const validationFacebook = facebook => {
    if (facebook.match("^(?![\\s\\S])") || facebook.match(("https:\\/\\/www\\.facebook\\.com\\/.{1,100}$"))) {
        return {errorText: "", errorState: false}
    } else {
